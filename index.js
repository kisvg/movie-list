...
function addTriggers(){
  ...
  document.getElementById('settings').onclick = async function(){
    await service_list.renderServiceList(globalThis.serviceList)
    if(globalThis.vpnMode){
      document.getElementById('vpn-mode').checked = true
    }
  }
...
async function getTmdb(data) {
  ...
  try{
    ...
    let services = await getServiceInfo(tmdbid, type)
...
async function getServiceInfo(tmdbid, type = "movie"){
  ...
  try{
    ...
    let data = initialdata.results
    // generated by ChatGPT, but seems pretty good:
...
async function applyFilters(){
  ...
  if (globalThis.doServiceFilter && globalThis.serviceList){
    let serviceArray = lowerArray(globalThis.serviceArray)
    if (globalThis.vpnMode){
      queries.push({key:`services.all_services_lower`,operator:'array-contains-any',value:serviceArray})
    }
    else{
      queries.push({key:`services.country_service_lower.${countryCode}`,operator:'array-contains-any',value:serviceArray})
    }
  }
...
async function openPop(movieId) {
  ...
  if (docSnap.exists()) {
    ...
    document.getElementById("pop-services").innerHTML=serviceHtml(data.services)
...
function serviceHtml(services){
  serviceArray = globalThis.serviceArray
  try{
    if(globalThis.vpnMode){
      if (services.all_services.length != 0){

        let html = ``
        let availableServices = services.all_services.filter(service => serviceArray.includes(service))
        if (availableServices.length != 0){
          html +=`
          <div class="available">
            ${availableServices.map(service => `
              <button class="service" onmouseover="document.getElementById('countries-${service}').classList.add('active')" onclick="document.getElementById('countries-${service}').classList.add('active')" onmouseout="document.getElementById('countries-${service}').classList.remove('active')">
                ${service}
              </button>
              <span class="countries" id="countries-${service}">${listCountries(services.service_country[service])}</span>
              `).join('')}
          </div>`
        }

        let unavailableServices = services.all_services.filter(service => !(serviceArray.includes(service)))
        if (unavailableServices.length != 0){ // add hiding functionality
          html +=`
          <div class="unavailable">
            ${unavailableServices.map(service => `<button class="service">${service}</button>`).join('')}
          </div>
        `
        }
        return html
      ...
      // not vpn mode
      let localServices = services.country_service[globalThis.countryCode]
      if(localServices.length != 0){
        let html = ``
        let availableServices = localServices.filter(service => serviceArray.includes(service))
        if (availableServices.length != 0){
          html += `          
          <div class="available">
            ${availableServices.map(service => `<div class="service">${service}</div>`).join('')}
          </div>`
        }

        let unavailableServices = localServices.filter(service => !(serviceArray.includes(service)))
        if (unavailableServices.length != 0){
          html += `          
          <div class="unavailable">
            ${unavailableServices.map(service => `<div class="service">${service}</div>`).join('')}
          </div>`
        }
    ...
    return `<div class="no-service faded"> Not streaming. </div>`
}

function listCountries(codes){
  const names = codes
    .map(code => globalThis.countryMap[code]['Name'])
...
async function updateList(){
  try{
    ...
    for (let docSnap of querySnapshot.docs) {
      ...
      let services = await getServiceInfo(data.tmdbid, data.type)
      await updateDoc(ref,{
        services: services,
      })
    }
